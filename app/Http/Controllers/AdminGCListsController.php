...
use Illuminate\Http\Request as IlluminateRequest;
	...
	class AdminGCListsController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
			...
			//$this->form[] = ["label"=>"Gc Decription","name"=>"gc_decription","type"=>"text","required"=>TRUE,"validation"=>"required|min:1|max:255"];
	        ...
	        */
			...
			if(CRUDBooster::getCurrentMethod() == 'getIndex'){
				...
				$this->index_button[] = ['label'=>'Scan QR','url'=>CRUDBooster::mainpath("scan_qr"),"icon"=>"fa fa-search", 'color'=>'info'];
				$this->index_button[] = ['label'=>'Upload GC List','url'=>CRUDBooster::mainpath("add"),"icon"=>"fa fa-plus", 'color'=>'info'];
			}
	    ...
	    */
		...
		public function getScanQR(IlluminateRequest $request) {
			...
			return $this->view('reedem_qr.scan_qr',$data);
		}
		...
		public function getEdit($id) {
			...
			$data['row'] = DB::table('g_c_lists')->where('id',$id)->first();
			
			// Generate QR Code
			$qrCodeData = $data['row']->email.'|'.$data['row']->id;

			//Please use view method instead view method from laravel
			return $this->view('reedem_qr.qr_redeem_section',$data);
